...
## 2026-06-25 - React List Filtering Performance
**Learning:** React state updates for input fields (like typing a search query) can block the main thread and cause typing lag if they synchronously trigger heavy filtering calculations on large client/job lists.
**Action:** Use `useDeferredValue(search)` in React 18+ to keep the input state updates high priority and defer the heavier, less-critical filtering operations, preventing UI sluggishness without needing a manual debounce.
