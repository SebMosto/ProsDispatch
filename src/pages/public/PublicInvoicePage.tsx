  ...
  if (error || !invoice) {
    return (
      <main className="flex min-h-[60vh] flex-col gap-2">
        <h1 className="text-xl font-semibold text-slate-900">{t('publicInvoice.notAvailable')}</h1>
        <p className="text-sm text-slate-600">{t('publicInvoice.invalidLink')}</p>
      </main>
    );
  }
    ...
    <main className="flex min-h-[60vh] flex-col gap-6">
      <header className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div>
            <p className="text-sm text-slate-600">{t('publicInvoice.invoiceNumber', { number: invoice.invoice_number })}</p>
            <h1 className="text-2xl font-semibold text-slate-900">{t('publicInvoice.invoiceFrom', { contractor: contractorLabel })}</h1>
          </div>
        </div>
        <div className="mt-3 text-lg font-semibold text-slate-900">
          {t('publicInvoice.totalDue')}: {formatCurrency(invoice.total_amount)}
        </div>
      </header>

      <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <h2 className="text-sm font-semibold text-slate-900">{t('publicInvoice.lineItems')}</h2>
        <div className="mt-3 space-y-3">
          {invoice.invoice_items?.length ? (
            ...
            <p className="text-sm text-slate-500">{t('publicInvoice.noItems')}</p>
          )}
        </div>
      </section>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex items-center justify-between text-sm text-slate-700">
          <span>{t('publicInvoice.subtotal')}</span>
        ...
        <div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-3 text-base font-semibold text-slate-900">
          <span>{t('publicInvoice.totalDue')}</span>
